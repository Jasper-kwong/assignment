...
pygame.display.set_caption("The Master Of Balance")

# initializes character images
image = pygame.image.load('main_character.png')
image_rect = image.get_rect()
image_rect.center = (400, 725)
random_enemy = pygame.image.load('boss_one_defend.png')
...
capybara_enemy = pygame.image.load('capybara.png')
...
world_one_background = pygame.image.load('world_one_background.jpg') 
world_one_background = pygame.transform.scale(world_one_background, (width, height))
world_two_background = pygame.image.load('world_two_background.png')
world_two_background = pygame.transform.scale(world_two_background, (width, height))
city_one_background = pygame.image.load('city_one_background.png')
city_one_background = pygame.transform.scale(city_one_background, (width, height))
selecting_moves_background = pygame.image.load('selecting_moves_background.png')
...
# Initializes move/technique variables
slot = ""
slot_number = ""
...
heal_sound = pygame.mixer.Sound('heal_sound.mp3')

# sets enemy's moves to enemy_one's stats
def enemy_one_stats():
    ...
    # Image for enemy
    enemy = pygame.image.load('image-removebg-preview (13).png')
...
# sets enemy's moves to enemy_two's stats
def enemy_two_stats():
    global enemy_attack_one, enemy_attack_two, enemy_attack_three, enemy_attack_four
    global enemy_damage_one, enemy_damage_two, enemy_damage_three, enemy_damage_four
    global enemy_cooldown_one, enemy_cooldown_two, enemy_cooldown_three, enemy_cooldown_four
    ...
    # Image of Enemy
    enemy = pygame.image.load('capybara.png')
    enemy_rect = image.get_rect()
...
# sets enemy's moves to enemy_three's stats
...
# sets enemy's moves to boss_one's stats
def boss_one_stats():
    global enemy_attack_one, enemy_attack_two, enemy_attack_three, enemy_attack_four
    global enemy_damage_one, enemy_damage_two, enemy_damage_three, enemy_damage_four
    global enemy_cooldown_one, enemy_cooldown_two, enemy_cooldown_three, enemy_cooldown_four
    ...
    enemy = pygame.image.load('boss_one.png')
...
# Draws level bar, and keeps track of player's level
def level_track():
    global player_level
...
# Draws battle platforms when in battle
def draw_ecllipse():
...
# Changes move slot to the selected move
# Applies correct stats to proper move slot
def punch(slot):
...
# Displays player's techniques and their cooldown bar when in battle
...
# Displays player and enemy's information such as name, level, and health when in battle
def player_bars():
    global enemy_health
    global damage_dealt
    global player_health
...
# User decides where they would like to place each move
def selecting_moves():
    global selecting_moves
    global question
    global Punch
    global slot_number
...
"""
The different locations/worlds where players are able to run around freely and fight enemies
Players are able to easily go from world to world by running to the edge of the world
"""
def world_one(x=100, y =900):
    ...
    while walking:
        ...
        if image_rect.x >= 1880:
            enemy_one_stats()
            background_music = pygame.mixer.music.load('battle_music.mp3')
            pygame.mixer.music.play(-1)
            encounter_loading()
            if fighting():
                background_music = pygame.mixer.music.load('background_music.mp3')
...
# Heals player to full health when they are below 100%
def heal():
    global player_health 
    global player_total_health
    global player_health_percent
...
# Opens the fighting interface where users will be fighting the enemies, using different attacks/abilities
def fighting(): 
    global enemy_attack_one, enemy_attack_two, enemy_attack_three, enemy_attack_four
    global enemy_damage_one, enemy_damage_two, enemy_damage_three, enemy_damage_four
    global enemy_cooldown_one, enemy_cooldown_two, enemy_cooldown_three, enemy_cooldown_four
    ...
    while Fight:
        ...
        if block_defense != 1:
            image = pygame.image.load('main_character_block.png')
